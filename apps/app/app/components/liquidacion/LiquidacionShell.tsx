...
const tabs = [
  { label: '📋 Liquidaciones', href: '/dashboard/liquidacion' },
  { label: '👥 Empleados', href: '/dashboard/liquidacion/empleados' },
] as const

export default function LiquidacionShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex gap-1 p-1 bg-white/[0.04] border border-white/[0.06] rounded-xl mb-8 w-fit">
        {tabs.map((tab) => {
          const isActive = tab.href === '/dashboard/liquidacion'
            ? pathname === '/dashboard/liquidacion' || /^\/dashboard\/liquidacion\/[^/]+$/.test(pathname)
