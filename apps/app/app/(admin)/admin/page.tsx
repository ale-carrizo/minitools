...
import { timeAgo, isOnline, ONLINE_THRESHOLD_MIN } from "@/lib/date";
...
export default async function AdminPage() {
  const [totalUsers, activeSubscriptions, trialSubscriptions, suspendedUsers, recentUsers, mrr, onlineNow] = await Promise.all([
    prisma.user.count(),
    ...
    prisma.user.count({ where: { lastActiveAt: { gt: new Date(Date.now() - ONLINE_THRESHOLD_MIN * 60 * 1000) } } }),
  ]);
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
        ...
        <StatCard label="En línea ahora" value={onlineNow} sub={`activos en los últimos ${ONLINE_THRESHOLD_MIN} min`} accent="#34D399" icon={<svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor"><circle cx="10" cy="10" r="6"/></svg>} />
            ...
            recentUsers.map((user) => (
              <div key={user.id} className="flex items-center gap-4 px-6 py-3.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="flex items-center gap-1 text-[11px] text-white/25 min-w-[64px] justify-end">
                    {isOnline(user.lastActiveAt) && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                    <span className={isOnline(user.lastActiveAt) ? "text-emerald-400" : ""}>
